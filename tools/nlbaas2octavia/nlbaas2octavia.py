...
def migrate_lb(LOG, n_session_maker, o_session_maker, lb_id):
    ...
    try:
        # Check if loadbalancer already exists in octavia
        o_lb = o_session.execute(
            "SELECT 1 FROM load_balancer "
            "WHERE ID = :id;",
            {'id': lb_id}).fetchone()

        if o_lb:
            LOG.warning("load balancer exists, skiping: %s", lb_id)
            return 0
...
def main():
    ...
    if not CONF.migration.trial_run:
